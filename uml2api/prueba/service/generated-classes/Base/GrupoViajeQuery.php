...
abstract class GrupoViajeQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'prueba', $modelName = '\\GrupoViaje', $modelAlias = null)
    {
     ...
     * </code>
     *
     * @param array[$idGrupo, $idViaje] $key Primary key to use for the query
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = GrupoViajeTableMap::getInstanceFromPool(serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1])]))))) {
    ...
    {
        $sql = 'SELECT idGrupo, idViaje FROM Grupo_viaje WHERE idGrupo = :p0 AND idViaje = :p1';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj->hydrate($row);
            GrupoViajeTableMap::addInstanceToPool($obj, serialize([(null === $key[0] || is_scalar($key[0]) || is_callable([$key[0], '__toString']) ? (string) $key[0] : $key[0]), (null === $key[1] || is_scalar($key[1]) || is_callable([$key[1], '__toString']) ? (string) $key[1] : $key[1])]));
        }
        $stmt->closeCursor();
    ...
    public function filterByGrupo($grupo, $comparison = null)
    {
        if ($grupo instanceof \Grupo) {
            ...
            return $this
                ->addUsingAlias(GrupoViajeTableMap::COL_IDGRUPO, $grupo->toKeyValue('PrimaryKey', 'Idgrupo'), $comparison);
        } else {
